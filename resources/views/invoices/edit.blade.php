...
<x-app-layout>
    ...
    <div class="mx-auto max-w-6xl space-y-8 px-4 py-6 sm:px-6 lg:px-8">
                    ...
                    <dl class="grid gap-4 sm:grid-cols-2 md:grid-cols-3 text-xs text-slate-600">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm">
                                @if(isset($lateFees['due_date']) && $lateFees['due_date'])
                                    {{ $lateFees['due_date']->format('M j, Y') }}
                                    @if(isset($lateFees['is_past_due']) && $lateFees['is_past_due'])
                                        <span class="ml-2 inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-semibold text-red-700">
                                            {{ $lateFees['days_overdue'] ?? 0 }} {{ trans_choice('day|days', $lateFees['days_overdue'] ?? 0) }} {{ __('overdue') }}
                                        </span>
                                    @endif
                                @else
                                    {{ __('Not available') }}
        ...
        <section class="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm">
            <h2 class="text-lg font-semibold text-slate-900">{{ __('How invoices  work') }}</h2>
            <div class="mt-3 space-y-3 text-sm text-slate-600">
                <p>{{ __('Each invoice is a snapshot of the related job logs at the moment the invoice was generated. Editing logs later will not update this invoice automatically.') }}</p>
                
                <div class="my-6 space-y-6">
                    <!-- Correct Flow (Green) -->
                    <div class="rounded-2xl border-2 border-emerald-200 bg-emerald-50/50 p-4">
                        <p class="mb-3 text-xs font-semibold uppercase tracking-wider text-emerald-700">{{ __('✓ Correct Data Flow') }}</p>
                        <div class="flex items-center justify-center gap-2">
                            <div class="flex flex-col items-center gap-2 rounded-xl border-2 border-emerald-400 bg-white px-4 py-3 shadow-sm">
                                <x-icon-job-flag class="h-8 w-8 text-emerald-600" />
                                <span class="text-xs font-bold text-emerald-700">JOB</span>
                            </div>
                            ...
                            <div class="flex flex-col items-center gap-2 rounded-xl border-2 border-emerald-400 bg-white px-4 py-3 shadow-sm">
                                ...
                                <span class="text-xs font-bold text-emerald-700">LOG</span>
                            </div>
                            <svg class="h-6 w-6 text-emerald-500" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24">
                            ...
                            <div class="flex flex-col items-center gap-2 rounded-xl border-2 border-emerald-400 bg-white px-4 py-3 shadow-sm">
                                ...
                                <span class="text-xs font-bold text-emerald-700">INVOICE</span>
                            </div>
                        </div>
                        <p class="mt-3 text-xs text-emerald-700">{{ __('Data flows from Job → Log → Invoice. Changes to Job/Log values will update') }} <span class="font-bold underline">{{ __('future') }}</span> {{ __('invoices  and has no effiect on previously generated invoices.') }}</p>
                    </div>

                    <!-- Incorrect Flow (Red) -->
                    <div class="rounded-2xl border-2 border-red-200 bg-red-50/50 p-4">
                        <p class="mb-3 text-xs font-semibold uppercase tracking-wider text-red-700">{{ __('✗ Does Not Flow Backwards') }}</p>
                        <div class="flex items-center justify-center gap-2">
                            <div class="flex flex-col items-center gap-2 rounded-xl border-2 border-red-300 bg-white px-4 py-3 shadow-sm opacity-60">
                                <svg class="h-8 w-8 text-red-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>
                                </svg>
                                <span class="text-xs font-bold text-red-600">INVOICE</span>
                            </div>
                            <svg class="h-6 w-6 text-red-400" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24">
                            ...
                            <div class="flex flex-col items-center gap-2 rounded-xl border-2 border-red-300 bg-white px-4 py-3 shadow-sm opacity-60">
                                ...
                                <span class="text-xs font-bold text-red-600">LOG</span>
                            </div>
                            <span class="text-2xl font-bold text-red-400 opacity-60">+</span>
                            <div class="flex flex-col items-center gap-2 rounded-xl border-2 border-red-300 bg-white px-4 py-3 shadow-sm opacity-60">
                                <x-icon-job-flag class="h-8 w-8 text-red-500" />
                                <span class="text-xs font-bold text-red-600">JOB</span>
                            </div>
                        </div>
                        <p class="mt-3 text-xs text-red-700">{{ __('Editing invoice values does NOT flow back to Logs or Jobs. Invoice edits only affect this specific invoice.') }}</p>
                    </div>
                </div>
